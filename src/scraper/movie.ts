...
export async function getMovie(link: string): Promise<LetterboxdMovie> {
    ...
    logger.debug(`getMovie(${movieUrl})`);
    ...
    logger.debug(`Fetched movie URL: ${movieUrl}`);
    return extractMovieFromHtml(link, html, movieUrl);
}

function extractMovieFromHtml(slug: string, html: string, movieUrl: string): LetterboxdMovie {
    const $ = cheerio.load(html);

    const name = extractName($);
    const tmdbId = extractTmdbId($);
    const imdbId = extractImdbId($);
    logger.debug(`From ${slug}: name=${name}, tmdbId=${tmdbId}, imdbId=${imdbId}`);
    const id = extractLetterboxdId($, movieUrl);
...
function extractLetterboxdId($: cheerio.CheerioAPI, movieUrl: string): number {
    ...
    try {
        ...
        if (typeof parsedFilmData !== 'object' || parsedFilmData === null || !('postered' in parsedFilmData)) {
            throw new Error('Missing `postered` field in resolvable poster data.');
        }
        const postedData = parsedFilmData.postered;
        if (typeof postedData !== 'object' || postedData === null || !('uid' in postedData) || typeof postedData.uid !== 'string') {
        ...
        if (!Number.isInteger(filmId)) {
            logger.error('Extracted Letterboxd ID is not a valid integer.', {
                filmId
            });
            throw new Error('Extracted Letterboxd ID is not a valid integer.');
        }
        logger.debug(`Extracted Letterboxd ID: ${filmId} from URL: ${movieUrl}`);
        return filmId;
    } catch (error) {
        logger.error('Failed to extract Letterboxd ID from HTML. This could be due to an unexpected HTML structure or missing data.', {
            error
        });
